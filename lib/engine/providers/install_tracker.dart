...
import '../setup/endpoint_registry.dart';
...
class InstallTracker {
  ...
  Future<void> warmup() async {
    if (_started) return;
    final devKey = AppIdentity.devKey;
    if (devKey.isEmpty) {
      _started = true;
      ...
      if (kDebugMode) {
        debugPrint('[ITR] dev key empty — skipping AppsFlyer init');
      }
      return;
    }
    ...
    try {
      ...
      if (kDebugMode) {
        debugPrint('[ITR] warmup error: $err');
        debugPrint('$st');
      }
      if (!_conversionGate.isCompleted) {
        _conversionGate.complete(<String, dynamic>{});
  ...
  void _handleConversion(dynamic raw) async {
    ...
    if (kDebugMode) {
      debugPrint('[ITR] conversion ${jsonEncode(data)}');
    }

    if (data['af_status'] == 'Organic') {
      await Future.delayed(
        Duration(seconds: AppIdentity.organicRefetchSeconds),
      );
  ...
  Future<Map<String, dynamic>?> _refetchGcd() async {
    try {
      ...
      final appId =
          Platform.isIOS ? AppIdentity.iosAppId : AppIdentity.packageName;
      ...
      final response = await httpClient.get(
        Uri.parse(url),
        headers: {
          'authorization': 'Bearer ${AppIdentity.devKey}',
        },
  ...
  Future<Map<String, dynamic>> composePayload({
    ...
    payload['bundle_id'] = AppIdentity.packageName;
    payload['store_id'] = AppIdentity.storeIdentifier;
    payload['os'] = Platform.isAndroid ? 'Android' : 'iOS';
    payload['locale'] = locale;
    if (pushToken != null && pushToken.isNotEmpty) {
      payload['push_token'] = pushToken;
    }
    if (AppIdentity.fbProjectId.isNotEmpty) {
      payload['firebase_project_id'] = AppIdentity.fbProjectId;
    }

    if (kDebugMode) {
      debugPrint('[ITR] payload ${jsonEncode(payload)}');
    }
    return payload;
  }
}
